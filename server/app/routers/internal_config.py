...
async def internal_notify_scan_completed(
    ...
    """Trigger automated scan completion & PDF email dispatch from Cloud Security worker."""
    ...
    res = await send_cloud_scan_completed_notification(
        ...
        pdf_report_filename=payload.pdf_filename or f"vrika_security_scan_{payload.scan_id[:8]}.pdf",
    )
